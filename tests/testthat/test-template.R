...
#    `functionName` is a function's name in your module (e.g., `fireSenseEvent1`).
# 2. Copy this file to the tests folder (i.e., ` # replace with empty string insteadfireSense/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireSense")
  ...
  parameters <- list(
    ...
    fireSense = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireSenseEvent1", envir = .GlobalEnv)) {
    simOutput <- fireSenseEvent1(mySim)
  } else {
    simOutput <- mySim$fireSenseEvent1(mySim)
  }
  ...
  if (exists("fireSenseEvent2", envir = .GlobalEnv)) {
    simOutput <- fireSenseEvent2(mySim)
  } else {
    simOutput <- mySim$fireSenseEvent2(mySim)
  }
